...
mod hoge {
    use crate::{
        errors::Error,
        geom_impls::{self, HomotopyConnector, LineConnector},
        topo_traits::*,
        Result,
    };
    ...
    #[inline(always)]
    pub fn rotated<T: Mapped<Matrix4>>(
        elem: &T,
        origin: Point3,
        axis: Vector3,
        angle: Rad<f64>,
...
fn partial_rsweep<T: MultiSweep<Point3, Curve, Surface>>(
    ...
    elem.multi_sweep(
        ...
        &move |pt, _| {
            geom_impls::circle_arc(*pt, origin, axis, angle / division as f64).to_same_geometry()
        },
        &move |curve, _| {
...
fn whole_rsweep<T: ClosedSweep<Point3, Curve, Surface>>(
    ...
    elem.closed_sweep(
        ...
        &move |surface| surface.transformed(trsl),
        &move |pt, _| {
            geom_impls::circle_arc(*pt, origin, axis, PI * 2.0 / DIVISION as f64).to_same_geometry()
        },
        &move |curve, _| {
            Surface::RevolutedCurve(Processor::new(RevolutedCurve::by_revolution(
                curve.clone(),
