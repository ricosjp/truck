...
macro_rules! toporedef {
    ($($type: ident, $is: ident, $into: ident, $to: ident),*) => {
        ...
        pub struct $type(monstertruck_modeling::$type);

        impl IntoWasm for monstertruck_modeling::$type {
...
#[wasm_bindgen]
pub struct StepHeaderDescriptor(monstertruck_step::out::StepHeaderDescriptor);
    ...
    ($type: ident) => {
        ...
        impl $type {
            ...
            pub fn from_json(data: &[u8]) -> Option<$type> {
                serde_json::from_reader::<_, monstertruck_modeling::$type>(data)
                .map_err(|e| println!("{e}"))
                .ok()
