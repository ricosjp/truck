...
impl<V> CurveDerivatives<V> {
        ...
        B: Fn(V, W) -> U,
    {
        ...
        (0..=max_order)
            .map(|n| self.combinatorial_derivative(other, &binomial, n))
            .collect()
    }
...
impl<V: Zero + Copy, const N: usize> TryFrom<[V; N]> for CurveDerivatives<V> {
    ...
    fn try_from(value: [V; N]) -> Result<Self, Self::Error> {
        if N == 0 {
            Err("empty array cannot convert to CurveDerivatives.")
        } else if N > MAX_DER_ORDER + 1 {
            Err("the length of CurveDerivatives must be less than MAX_DER_ORDER + 1.")
        } else {
            let mut array = [V::zero(); MAX_DER_ORDER + 1];
...
impl<V: Zero + Copy> TryFrom<&[V]> for CurveDerivatives<V> {
    type Error = &'static str;
    fn try_from(value: &[V]) -> Result<Self, Self::Error> {
        if value.is_empty() {
            Err("empty slice cannot convert CurveDerivatives.")
...
impl<V: Zero + Copy> FromIterator<V> for CurveDerivatives<V> {
...
impl<V: VectorSpace> std::ops::Mul<V::Scalar> for CurveDerivatives<V> {
    type Output = Self;
...
impl<V: VectorSpace> std::ops::Div<V::Scalar> for CurveDerivatives<V> {
    type Output = Self;
...
impl<V> AbsDiffEq for CurveDerivatives<V>
where
    V: AbsDiffEq,
    V::Epsilon: Copy,
{
...
#[derive(Clone, Copy, PartialEq)]
pub struct SurfaceDerivatives<V> {
    array: [[V; MAX_DER_ORDER + 1]; MAX_DER_ORDER + 1],
    max_order: usize,
}

impl<V> SurfaceDerivatives<V> {
    ...
    /// ```
    /// use truck_base::derivatives::*;
    /// let derivs = SurfaceDerivatives::<f64>::new(5);
    /// let slices = derivs.slice_iter().collect::<Vec<_>>();
    ...
    /// ```
    /// use truck_base::derivatives::*;
    /// let mut derivs = SurfaceDerivatives::<f64>::new(5);
    /// let slices = derivs.slice_iter_mut().collect::<Vec<_>>();
    /// assert_eq!(slices.len(), 6);
    /// assert_eq!(slices[0].len(), 6);
    /// assert_eq!(slices[1].len(), 5);
    /// assert_eq!(slices[2].len(), 4);
    ...
    /// Returns one order of composite derivatives.
    #[inline]
    pub fn composite_der(
        &self,
        curve_ders: &CurveDerivatives<Vector2<V::Scalar>>,
        order: usize,
    ) -> V
    where
        ...
        V::Scalar: BaseFloat,
    {
        self.composite_derivative(curve_ders, order)
    }
        ...
        V::Scalar: BaseFloat,
    {
        ...
        let mut res = CurveDerivatives::new(max_order);
        res[0] = self[0][0];
...
impl<V> std::ops::Index<usize> for SurfaceDerivatives<V> {
    type Output = [V];
...
impl<V> std::ops::IndexMut<usize> for SurfaceDerivatives<V> {
...
impl<V: Zero + Copy> TryFrom<&[&[V]]> for SurfaceDerivatives<V> {
    type Error = &'static str;
    fn try_from(value: &[&[V]]) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err("Empty array cannot convert to `SurfaceDerivatives`.");
        }
        let mut array = [[V::zero(); MAX_DER_ORDER + 1]; MAX_DER_ORDER + 1];
...
impl<V: VectorSpace> std::ops::Mul<V::Scalar> for SurfaceDerivatives<V> {
    type Output = Self;
    fn mul(self, rhs: V::Scalar) -> Self::Output {
        let mut array = self.array;
...
impl<V: VectorSpace> std::ops::Div<V::Scalar> for SurfaceDerivatives<V> {
    type Output = Self;
    fn div(self, rhs: V::Scalar) -> Self::Output {
        let mut array = self.array;
        array.iter_mut().flatten().for_each(|a| *a = *a / rhs);
...
impl<V> AbsDiffEq for SurfaceDerivatives<V>
where
    V: AbsDiffEq,
    V::Epsilon: Copy,
{
...
fn surface_ders_debug() {
    let ders = SurfaceDerivatives::<f64>::new(2);
