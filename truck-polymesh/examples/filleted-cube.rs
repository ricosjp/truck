...
fn main() {
    ...
    obj::write(&handler.create_mesh_by_face_indices(&planes), file).unwrap();
    ...
    obj::write(&handler.create_mesh_by_face_indices(&lower), file).unwrap();
    let file = std::fs::File::create("upper.obj").unwrap();
    obj::write(&handler.create_mesh_by_face_indices(&upper), file).unwrap();
}
