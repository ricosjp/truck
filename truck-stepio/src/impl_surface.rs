...
macro_rules! sub_impl_surface {
    ($mod: tt) => {
        ...
        $crate::impl_try_from!(
            impl TryFrom<&$mod::ElementarySurfaceAny> for ElementarySurface {
                fn try_from(
                    es: &$mod::ElementarySurfaceAny,
                ) -> Result<ElementarySurface, ExpressParseError> {
                    use $mod::ElementarySurfaceAny as ES;
                    match es {
                        ES::ElementarySurface(_) => Err("not enough data!".to_string()),
                        ES::Plane(x) => Ok(Self::Plane((&**x).into())),
                        ES::CylindricalSurface(x) => Ok(Self::RevolutedLine((&**x).into())),
                        ES::ConicalSurface(x) => Ok(Self::RevolutedLine((&**x).into())),
                        ES::SphericalSurface(x) => Ok(Self::Sphere((&**x).into())),
                        ES::ToroidalSurface(x) => Ok(Self::ToroidalSurface(
                            AsRef::<$mod::ToroidalSurface>::as_ref(&**x).into(),
                        )),
                    }
                }
            }
            ...
            impl TryFrom<&$mod::SurfaceAny> for Surface {
                fn try_from(surface: &$mod::SurfaceAny) -> Result<Self, ExpressParseError> {
                    use $mod::SurfaceAny as SA;
                    match surface {
                        SA::Surface(_) => Err("not enough data!".to_string()),
                        SA::ElementarySurface(x) => Ok(Self::ElementarySurface((&**x).try_into()?)),
                        SA::SweptSurface(x) => Ok(Self::SweptCurve((&**x).try_into()?)),
                        _ => unimplemented!(),
                    }
                }
            }
