...
fn test_split_closed_face_cylinder_with_hole() {
    ...
    assert_eq!(shell.edges.len(), 9);
    assert_eq!(shell.edges[7].vertices, (3, 5));
    let curve0 = &shell.edges[7].curve;
    let curve1 = Line(Point3::new(-1.0, 0.0, 1.0), Point3::new(-1.0, 0.0, 0.75));
    for i in 0..=10 {
        let t = i as f64 / 10.0;
        assert_near!(curve0.subs(t), curve1.subs(t));
    }
    assert_eq!(shell.edges[8].vertices, (4, 1));
    let curve0 = &shell.edges[8].curve;
...
fn test_split_closed_face_cylinder_with_rotated_hole() {
    ...
    assert_eq!(shell.edges[5].vertices, (4, 7));
    assert_eq!(shell.edges[6].vertices, (5, 6));
    assert_eq!(shell.edges[7].vertices, (6, 4));
    assert_eq!(shell.edges[8].vertices, (7, 5));
    assert_eq!(shell.edges[9].vertices, (3, 7));
    let curve0 = &shell.edges[9].curve;
    let curve1 = Line(Point3::new(-1.0, 0.0, 1.0), Point3::new(-1.0, 0.0, 0.75));
    for i in 0..=10 {
        let t = i as f64 / 10.0;
        assert_near!(curve0.subs(t), curve1.subs(t));
    }
    assert_eq!(shell.edges[10].vertices, (6, 1));
    let curve0 = &shell.edges[10].curve;
    let curve1 = Line(Point3::new(-1.0, 0.0, 0.25), Point3::new(-1.0, 0.0, 0.0));
    for i in 0..=10 {
        let t = i as f64 / 10.0;
        assert_near!(curve0.subs(t), curve1.subs(t));
